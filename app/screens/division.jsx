...
const Divisions = ({ navigation }) => {
    ...
    const handleKeyPress = (keyValue) => {
        ...
        if (keyValue === 'backspace') {
            ...
            let correct_conversion = Percent / Number;
            correct_conversion = Math.floor(correct_conversion);
            if (inputText == correct_conversion) {
                setCorrect(Correct + 1);
            }
            // chenge value of percent and number
            if (Round === 1) {
                let random1 = Math.floor(Math.random() * 50) + 11;
                setNumber(random1);
                let random2 = Math.floor(Math.random() * 50) + 11;
                setPercent(random2);
            }
            else if (Round === 2) {
                let random1 = Math.floor(Math.random() * 50) + 11;
    ...
    if (isGameFinished) {
        return (
            <View style={{ flex: 1, alignItems: 'center' }}>
                <ImageBackground source={require('../../assets/imgs/game_back/game_division_fullscreen.webp')} style={{ width: '100%', height: '100%' }}>
    ...
    if (isBlackBackground) {
        ...
        return (
            <View style={{ flex: 1, alignItems: 'center' }}>
                <ImageBackground source={require('../../assets/imgs/game_back/game_division_fullscreen.webp')} style={{ width: '100%', height: '100%' }}>
                    <TouchableOpacity onPressOut={() => Alert.alert('HINT', 'Divide the number on the left by the number on the right. Divide means to split into n equal parts.')}
                    >
                    ...
                    >ROUND {Round}/4</Text>
                    <Text style={{ color: 'white', fontSize: 48, fontFamily: 'Dino-Normal', position: 'absolute', justifyContent: 'center', top: 166, left: 88, alignSelf: 'center' }}
                    >{Percent} ÷</Text>
                    <Text style={{ color: 'white', fontSize: 48, fontFamily: 'Dino-Normal', position: 'absolute', justifyContent: 'center', top: 166, left: 250, alignSelf: 'center' }}
                    >{Number}</Text>
                    <Text style={{ color: 'white', fontSize: 36, fontFamily: 'Dino-Medium', position: 'absolute', justifyContent: 'center', top: 180, left: 188, alignSelf: 'center' }}
                    >BY</Text>
